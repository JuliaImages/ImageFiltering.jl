...
@testset "Planned FFT with IIR filters should error" begin
    imgf = rand(Float64, 50, 50)
    
    # IIRGaussian is an IIR filter
    iir_kernel = KernelFactors.IIRGaussian(Float64, (3.0,))
    @test_throws ArgumentError planned_fft(imgf, iir_kernel, "replicate")
    
    # TriggsSdika tuple
    ts_kernel = KernelFactors.IIRGaussian(Float64, (3.0, 3.0))
    @test_throws ArgumentError planned_fft(imgf, ts_kernel, Pad(:replicate))
end

@testset "Plan Reuse Across Multiple Images" begin
    @testset "Reuse with same-size Float64 images" begin
        # Create multiple images of the same size
        images = [rand(Float64, 50, 50) for _ in 1:5]
        # Add some structure to make filtering meaningful
        for img in images
            img[20:30, 20:30] .= 1.0
        end
        ...
        # Create plan once
        planned_alg = planned_fft(images[1], kernel, border)
        
        # Reuse plan for all images
        for (i, img) in enumerate(images)
            result_planned = imfilter(img, kernel, border, planned_alg)
            result_standard = imfilter(img, kernel, border, Algorithm.FFT())
            @test result_planned ≈ result_standard rtol=1e-10
        end
    end
    
    @testset "Reuse with same-size Gray images" begin
        images = [rand(Gray{Float64}, 40, 60) for _ in 1:3]
    ...
    @testset "Wrong-size image should error" begin
        img1 = rand(Float64, 50, 50)
        img2 = rand(Float64, 60, 60)  # Different size
        kernel = rand(Float64, 3, 3)
        border = Fill(0.0)
        
        planned_alg = planned_fft(img1, kernel, border)
        
        # First image should work
        result1 = imfilter(img1, kernel, border, planned_alg)
        @test result1 isa Array{Float64, 2}
        
        # Wrong-size image should throw an error
        @test_throws Exception imfilter(img2, kernel, border, planned_alg)
    end
    
    @testset "Different element type with same size" begin
        img_f64 = rand(Float64, 50, 50)
        img_f32 = rand(Float32, 50, 50)
        kernel = rand(Float64, 3, 3)
        border = Fill(0.0)
        
        planned_alg = planned_fft(img_f64, kernel, border)
        
        # Float32 image with Float64 plan should work (gets converted)
        result = imfilter(img_f32, kernel, border, planned_alg)
        @test result isa Array
        
        # Compare with standard FFT
        result_standard = imfilter(img_f32, kernel, border, Algorithm.FFT())
        @test result ≈ result_standard rtol=1e-6
    end
end

@testset "Concurrent Filtering with Planned FFT" begin
    # Only run if we have multiple threads
    if Threads.nthreads() > 1
        @testset "Concurrent Float64 filtering" begin
            n_images = 10
            images = [rand(Float64, 100, 100) for _ in 1:n_images]
            # Add structure to images
            for img in images
                img[40:60, 40:60] .= rand()
            end
            ...
            # Create plan once
            planned_alg = planned_fft(images[1], kernel, border)
            results = Vector{Any}(undef, n_images)
            
            # Filter concurrently
            Threads.@threads for i in 1:n_images
                results[i] = imfilter(images[i], kernel, border, planned_alg)
            end
            
            # Verify correctness
            for i in 1:n_images
                expected = imfilter(images[i], kernel, border, Algorithm.FFT())
                @test results[i] ≈ expected rtol=1e-8
            end
        end
        
        @testset "Concurrent Gray image filtering" begin
            n_images = 8
            images = [rand(Gray{Float64}, 80, 80) for _ in 1:n_images]
            kernel = Kernel.gaussian((3, 3))
            border = Fill(Gray(0.0))
        ...
        @info "Skipping concurrent tests (only 1 thread available). Run with multiple threads to test concurrent filtering."
    end
end

@testset "Plan Reuse Validation" begin
    @testset "Plan structure validation" begin
        img = rand(Float64, 50, 50)
        kernel = rand(Float64, 3, 3)
        border = Fill(0.0)
        
        planned_alg = planned_fft(img, kernel, border)
        
        # Verify it's an Algorithm.FFT with plans
        @test planned_alg isa Algorithm.FFT
        @test planned_alg.plan1 !== nothing
        @test planned_alg.plan2 !== nothing
        @test planned_alg.plan3 !== nothing
        
        # Verify plans are callable functions
        @test planned_alg.plan1 isa Function
        @test planned_alg.plan2 isa Function
        @test planned_alg.plan3 isa Function
    end
    
    @testset "Compare planned vs unplanned FFT" begin
        img = rand(Float64, 100, 100)
        img[45:55, 45:55] .= 1.0
        kernel = Kernel.gaussian((7, 7))
        border = Pad(:symmetric)
        ...
        # Unplanned FFT
        result_unplanned = imfilter(img, kernel, border, Algorithm.FFT())
        
        # Results should be very close (within numerical precision)
        @test result_planned ≈ result_unplanned rtol=1e-10
        
        # Verify we get the same axes
        @test axes(result_planned) == axes(result_unplanned)
    end
    
    @testset "Offset arrays with planned FFT reuse" begin
        images = [OffsetArray(rand(Float64, 50, 50), (0:49, 1:50)) for _ in 1:3]
        kernel = rand(Float64, 3, 3)
        border = Fill(0.0)
        ...
        for img in images
            ...
            @test axes(result_planned) == axes(img)
            @test axes(result_standard) == axes(img)
