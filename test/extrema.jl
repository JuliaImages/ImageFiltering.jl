@testset "extrema" begin
    @testset "local extrema" begin
        ...
        @test findlocalmaxima(A; dims=2) == [CartesianIndex((1,5)),CartesianIndex((2,5)),CartesianIndex((5,5))]
        @test findlocalmaxima(A; dims=2, edges=false) == [CartesianIndex((2,5)),CartesianIndex((5,5))]
        A = zeros(Int, 9, 9, 9); A[[1:2;5],5,5].=1
        @test findlocalmaxima(A) == [CartesianIndex((5,5,5))]
        @test findlocalmaxima(A; dims=2) == [CartesianIndex((1,5,5)),CartesianIndex((2,5,5)),CartesianIndex((5,5,5))]
        @test findlocalmaxima(A,dims=2, edges=false) == [CartesianIndex((2,5,5)),CartesianIndex((5,5,5))]
        A = zeros(Int, 9, 9); A[[1:2;5],5].=-1
